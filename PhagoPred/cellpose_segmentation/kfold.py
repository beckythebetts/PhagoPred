...
class CellposeKfold:
    ...
    def __init__(self, directory):
        ...
        self.masks = sorted(
            [mask for mask in (self.directory / 'all').glob('*mask*')])
    ...
    def split_all(self, num_val=1):
        for i in range(len(self.ims)):
            ...
            val_ims = [
                self.ims[(i + x) % len(self.ims)] for x in range(num_val)
            ]
    ...
    def train_eval_datasets(self):
        for dataset_dir in self.directory.glob('*model*'):
            if dataset_dir.name != 'all':
                ...
                eval.cellpose_eval(dataset_dir / 'validate',
                                   model=dataset_dir / 'models' / 'model')
    ...
    def get_results(self):
        ...
        self.means, self.stds = results.groupby(
            level=0).mean(), results.groupby(level=0).std()
    ...
    def plot_results(self):
        ...
        for ax, metric in zip(axs, metrics):
            ...
            ax.fill_between(thresholds,
                            self.means[metric] - self.stds[metric],
                            self.means[metric] + self.stds[metric],
                            color='red',
                            alpha=0.5,
                            edgecolor='none')
    ...
    def plot_losses(self):
        ...
        test_losses = []
        file_names = [
            file / 'losses.txt' for file in self.directory.iterdir()
            if file.name != 'all'
        ]
        ...
        test_iterations, test_means, test_stds = np.array(
            [[it, mean, std]
             for it, mean, std in zip(iterations, test_means, test_stds)
             if mean != 0]).transpose()
        plt.rcParams["font.family"] = 'serif'

        plt.plot(test_iterations,
                 test_means,
                 color='red',
                 label='Validation loss')
        plt.fill_between(test_iterations,
...
def plot_training_data(training_data_values=[0, 2, 4]):
    directories = [
        Path('cellpose_Models') / f'kfold_manual_validation_{i}'
        for i in training_data_values
    ]
    means = [
        pd.read_csv(directory / 'results_means.txt', sep='\t', index_col=0)
        for directory in directories
    ]
    stds = [
        pd.read_csv(directory / 'results_stds.txt', sep='\t', index_col=0)
        for directory in directories
    ]
    ...
    im_views = [
        plt.imread(directory / 'test_with_00' / 'validate' /
                   '00_view.png')[200:800, 200:800]
        for directory in directories
    ]
