...
class CellViewer(QWidget):
    ...
    def __init__(self,
        ...
        self.feature_names = [
            k for k in self.hdf5_file['Cells']['Phase'].keys()
            if k not in ('Images', 'First Frame', 'Last Frame', 'X', 'Y',
                         'CellDeath', 'Macrophage', 'Dead Macrophage')
        ]

        # Create plots widget with checkboxes
        self.plots_widget = FeaturePlotsWidget(self.feature_names)
        self.plots_dock = self.viewer.window.add_dock_widget(self.plots_widget,
                                                             area='bottom')
    ...
    def on_load_finished(self, phase_data, epi_data, mask):
        ...
        self.viewer.add_image(self.phase_data,
                              name=f'Phase {self.cell_idx}',
                              colormap='gray',
                              translate=(self.first_frame, 0, 0))
        self.viewer.add_image(self.epi_data,
                              name=f'Epi {self.cell_idx}',
                              blending='additive',
                              colormap='red',
                              opacity=0.5,
                              translate=(self.first_frame, 0, 0))
        # cell_mask = (mask == self.cell_idx).astype(np.int32)
        labels_layer = self.viewer.add_labels(mask,
                                              name=f'Cell {self.cell_idx}',
                                              colormap={1: [255, 255, 0, 255]},
                                              translate=(self.first_frame, 0,
                                                         0))
        ...
        for feat, pw in self.plots_widget.plot_widgets.items():
            vline = pg.InfiniteLine(pos=self.first_frame,
                                    angle=90,
                                    pen=pg.mkPen('r', width=2))
            pw.addItem(vline)
            self.vlines[feat] = vline
            if not np.isnan(self.cell_death):
                vline_death = pg.InfiniteLine(pos=self.cell_death,
                                              angle=90,
    ...
    def update_feature_plots(self):
        for feat in self.feature_names:
            y_data = self.hdf5_file['Cells']['Phase'][feat][
...
class CellLoaderThread(QThread):
    ...
    def run(self):
        try:
            ...
            phase_data = np.empty(
                (self.n_frames, self.frame_size, self.frame_size))
            epi_data = np.empty(
                (self.n_frames, self.frame_size, self.frame_size))
            mask = np.empty((self.n_frames, self.frame_size, self.frame_size),
                            dtype=np.int32)

            # with h5py.File(self.hdf5_file, 'r') as f:
            x_centres = self.hdf5_file['Cells']['Phase']['X'][
                self.first_frame:self.last_frame, self.cell_idx]
            y_centres = self.hdf5_file['Cells']['Phase']['Y'][
                self.first_frame:self.last_frame, self.cell_idx]
