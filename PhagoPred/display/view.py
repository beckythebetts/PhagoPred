...
def show_tracked_images(first_frame=0, last_frame=50):
    ...
    with h5py.File(hdf5_file, 'r') as f:
        ...
        xs, ys = tools.get_features_ds(f['Cells']['Phase'], 'x'), tools.get_features_ds(f['Cells']['Phase'], 'y')
    ...
    for i, im in enumerate(tracked):
        sys.stdout.write(
            f'\r Labelling Frame {i + 1}')
        sys.stdout.flush()
        cv2.putText(im, text=f'frame: {i+first_frame} | time: {(i+first_frame)*SETTINGS.TIME_STEP}/s', org=(20, SETTINGS.IMAGE_SIZE[1]-20), fontFace=cv2.FONT_HERSHEY_COMPLEX, 
                    fontScale=3, color=(255, 255, 255), thickness=3)
        for idx, (x, y) in enumerate(zip(xs[i+first_frame], ys[i+first_frame])):
            if not np.isnan(x) and idx!=0:
                color = [int(color) for color in LUT[idx]]
                cv2.putText(im, text=str(idx), org=(int(x), int(y)), fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=1, color=color, thickness=2)
    tracked_image = ij.py.to_dataset(tracked, dim_order=['t', 'row', 'col', 'ch'])
    ij.ui().show(tracked_image)
    ij.py.run_macro(macro='run("Make Composite")')
    time.sleep(99999)

def show_cell_images(cell_idx, first_frame=0, last_frame=50, frame_size=500):
