...
from PhagoPred.feature_extraction.texture.fitting import TextureFit
...
def frame_by_frame_features(phase_masks, phase_ims, cell_batchsize, num_cells, morphology_fit, texture_fit):
    intensity_means = []
    intensity_variances = []
    perimeters = []
    morph_scores = []
    ...
    for i, (phase_mask, phase_im) in enumerate(zip(phase_masks, phase_ims)):
        ...
        morph_scores.append(morphology_fit.apply(i))
        text_scores.append(texture_fit.apply(i))
    return np.concatenate((np.stack((np.array(intensity_means), np.array(intensity_variances), np.array(perimeters)), axis=2), np.array(morph_scores), np.array(text_scores)), axis=2)
...
def derived_features(f, first_frame, last_frame):
    ...
    areas, perimeters = tools.get_features_ds(f['Cells']['Phase'], 'area')[first_frame:last_frame], tools.get_features_ds(f['Cells']['Phase'], 'perimeter')[first_frame:last_frame]
    x0, y0 = all_xs[~np.isnan(all_xs)][0], all_ys[~np.isnan(all_ys)][0]
    not_first = 0 if first_frame == 0 else 1
    # if first_frame != 0:
    #     first_frame -= 1
    ...
    phago_densities_100 = np.sum(phago_phago_distances < 100, axis=1) - 1
    phago_densities_250 = np.sum(phago_phago_distances < 250, axis=1) - 1
    phago_densities_500= np.sum(phago_phago_distances < 500, axis=1) - 1
    ...
    perimeter_over_area = perimeters/areas
    return np.stack((speeds, 
                     displacements[not_first:], 
                     displacement_speeds, 
                     phago_densities_100[not_first:],
                     phago_densities_250[not_first:],
                     phago_densities_500[not_first:],
                     perimeter_over_area), axis=2)


def get_features(file=SETTINGS.DATASET, frame_batchsize=500, cell_batchsize=100):
    with h5py.File(file, 'r+') as f:
        # First need to fit pca and kmeans clustering models
        print('Fitting morphology classifier ...')
        morphology_fit = MorphologyFit()
        morphology_fit.fit()

        print('Fitting texture classfier ...')
        ...
        morph_names = [f'morphology{i}' for i in range(SETTINGS.KMEANS_CLUSTERS)]
        text_names = [f'texture{i}' for i in range(SETTINGS.KMEANS_CLUSTERS)]
        # modes = np.arange(SETTINGS.KMEANS_CLUSTERS).astype(str)
        frame_by_frame_feature_names = np.append(['intensity_mean', 'intensity_variance', 'perimeter'], np.append(morph_names, text_names))
        derived_feature_names = ['speed', 'displacement', 'displacement_speed', 'phagocyte_density_100', 'phagocyte_density_250', 'phagocyte_density_500', 'perimeter_over_area']
        all_new_feature_names = np.append(frame_by_frame_feature_names, derived_feature_names)
        f['Cells']['Phase'].attrs['features'] = f['Cells']['Phase'].attrs['features'][:3]
        ...
        for first_frame in range(0, num_frames, frame_batchsize):
            ...
            f['Cells']['Phase'][first_frame:last_frame, :, 3:] = np.concatenate((frame_by_frame_features(phase_mask, phase_im, cell_batchsize, num_cells, morphology_fit, texture_fit), 
