...
def frame_by_frame_features(phase_masks, phase_ims, cell_batchsize, num_cells):
    ...
    for phase_mask, phase_im in zip(phase_masks, phase_ims):
    ...
    return np.stack((np.array(intensity_means), np.array(intensity_variances), np.array(perimeters)), axis=2)
...
def derived_features(f, first_frame, last_frame):
    ...
    phago_densities = np.sum(phago_phago_distances < 150, axis=1)
...
def get_features(file=SETTINGS.DATASET, frame_batchsize=25, cell_batchsize=50):
    with h5py.File(file, 'r+') as f:
        ...
        f['Cells']['Phase'].attrs['features'] = np.append(f['Cells']['Phase'].attrs['features'], ['intensity_mean', 'intensity_variance', 'perimeter', 'speed', 'displacement', 'displacement_speed', 'phagocyte density', 'pathogen_density'])
        ...
        for first_frame in range(0, num_frames, frame_batchsize):
            ...
            #intensity_np(phase_mask, phase_im)s
            f['Cells']['Phase'][first_frame:last_frame, :, 3:] = np.concatenate((frame_by_frame_features(phase_mask, phase_im, cell_batchsize, num_cells), derived_features(f, first_frame, last_frame)), axis=2)
