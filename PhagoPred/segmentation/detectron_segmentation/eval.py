...
def evaluator(directory=SETTINGS.MASK_RCNN_MODEL,
    ...
    cfg.MODEL.WEIGHTS = segment.resolve_weights(
        config_directory, weights_name)  # path to the model we just trained
...
class Evaluator:
    ...
    def __init__(self,
                 dataset_dir: Path = (SETTINGS.MASK_RCNN_MODEL /
                                      'Fine_Tuning_Data'),
                 model_dir: Path = (SETTINGS.MASK_RCNN_MODEL / 'Model'),
                 eval_mode: str = "metrics",
                 weights_name: str = 'model_final.pth') -> None:
    ...
    def eval(self) -> None:
        ...
        for im_name in tqdm(list(
            (self.dataset_dir / 'validate' / 'images').iterdir()),
                            desc="Evaluating "):
            ...
            pred_masks = segment.seg_image(cfg_dir=self.model_dir,
                                           im=frame_processed,
            ...
            if pred_masks is None:
                ...
                pred_masks = {
                    category: np.zeros_like(frame_processed[:, :, 0])
                    for category in self.categories
                }
            true_masks = mask_funcs.coco_to_masks(coco_file=self.dataset_dir /
    ...
    def _eval_metrics(self, im_name, im, pred_masks, true_masks):
        ...
        results.to_csv(self.eval_dir / f'{im_name.stem}_all_results.txt',
                       sep='\t')

        for category in pred_masks.keys():
            view = tools.show_segmentation(im, pred_masks[category],
                                           true_masks[category])
            plt.imsave(self.eval_dir / f'{im_name.stem}_{category}_view.png',
                       view)

            pred_mask_im = Image.fromarray(pred_masks[category].astype(
                np.int32),
                                           mode='I')
            pred_mask_im.save(self.eval_dir /
                              f'{im_name.stem}_{category}_pred_mask.png')

            results = self.prec_recall_curve(true_masks[category],
                                             pred_masks[category])
            results.to_csv(self.eval_dir /
                           f'{im_name.stem}_{category}_results.txt',
                           sep='\t')
    ...
    def _largest_mask_to_class(self, mask_dict: dict) -> int:
        ...
        for idx, category in enumerate(
                self.categories[:-1]):  # exclude "No Cell"
    ...
    def plot_confusion_matrix(self, y_true: list[int], y_pred: list[int]):
        ...
        ax.axhline(no_cell_idx - 0.5,
                   color='gray',
                   linestyle='--',
                   linewidth=1)
        ax.axvline(no_cell_idx - 0.5,
                   color='gray',
                   linestyle='--',
                   linewidth=1)
        ax.axhline(no_cell_idx + 0.5,
        ...
        normalised_cm = confusion_matrix(y_true,
                                         y_pred,
                                         labels=labels,
                                         normalize='true')
        print(normalised_cm)
...
def main():
    ...
    evaluator = Evaluator(
        dataset_dir=Path(
            "/home/ubuntu/PhagoPred/PhagoPred/detectron_segmentation/models/27_05_mac/kfold_fine_tune/Training_Data"
        ),
        model_dir=Path(
            "PhagoPred/detectron_segmentation/models/27_05_mac/kfold_no_fine_tune/split_1/Model"
        ),
        eval_mode="metrics",
    )
    evaluator.eval()
