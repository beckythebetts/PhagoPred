...
from PhagoPred.segmentation.unet_segmentation.dataset import UNetDataset_inference, UNetDataset_train
...
def segment(dataset=SETTINGS.DATASET, model=SETTINGS.UNET_MODEL):
    with h5py.File(dataset, 'r+') as f:
        ...
        unet_dataset_loader = torch.utils.data.DataLoader(unet_dataset,
                                                          batch_size=1)
        ...
        with torch.no_grad():
            for frame, image in enumerate(unet_dataset_loader):
                ...
                image = tools.unpad_image(
                    image[0, 0, :, :], unet_dataset.padding_width,
                    unet_dataset.padding_height).cpu().numpy()
                init_output = tools.unpad_image(
                    pred_mask[0, 0, :, :], unet_dataset.padding_width,
                    unet_dataset.padding_height).cpu().numpy()
                # print(pred_mask.shape)
                pred_mask = pred_mask > 0
                # print(pred_mask.shape)
                pred_mask = tools.unpad_image(
                    pred_mask[0, 0, :, :], unet_dataset.padding_width,
                    unet_dataset.padding_height).cpu().numpy()
                joined = np.concatenate((init_output, pred_mask, image),
                                        axis=1)
                plt.matshow(joined)
                plt.show()
