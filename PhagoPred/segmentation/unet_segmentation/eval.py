...
def eval(dir=SETTINGS.UNET_MODEL):
    ...
    with torch.no_grad():
        ...
        for i, (image, mask) in enumerate(val_loader):
            ...
            mask, binary_pred = mask.type(torch.bool), binary_pred.type(
                torch.bool)
            view = tools.show_semantic_segmentation(
                image[0, 0, :, :], binary_pred[0, 0, :, :],
                mask[0, 0, :, :]).cpu().numpy().astype(np.uint8)
            view = Image.fromarray(view, 'RGB')
            view.save(dir / f'view_segmentation_{i}.png')

            view_output = tools.min_max_normalise(pred[0,
                                                       0, :, :].cpu().numpy())
            view_output = Image.fromarray((view_output * 255).astype(np.uint8),
                                          'L')
            view_output.save(dir / f'view_output_{i}.png')

            ious.append(
                mask_funcs.cal_iou(binary_pred.cpu().numpy()[0],
                                   mask.cpu().numpy()[0]))
        with open(dir / 'iou.txt', 'w') as f:
            f.write('\t'.join(map(str, ious)) +
