...
def cellpose_eval(directory,
                  model=SETTINGS.CELLPOSE_MODEL / 'models' / 'model',
                  thresholds=np.arange(0.5, 1.0, 0.05)):
    use_GPU = core.use_gpu()
    io.logger_setup()
    model = models.CellposeModel(gpu=use_GPU, pretrained_model=str(model))
    im_names = [im.stem[:4] for im in directory.iterdir() if 'im' in im.stem]
    validation_ims = [
        io.imread(im) for im in directory.iterdir() if 'im' in im.name
    ]
    channels = [0, 0]
    ...
    for pred, im_name in zip(preds, im_names):
        Image.fromarray(pred.astype(np.uint16)).save(
            str(directory / f'{im_name}pred.png'))

        #plt.imsave(str(directory/f'{im_name}pred.png'), pred, cmap='gray')
    true_masks = [
        io.imread(im) for im in directory.iterdir() if 'mask' in im.name
    ]
    # NOTE, this is not AP as defined elsewhere
    APs, TPs, FPs, FNs = metrics.average_precision(true_masks,
                                                   preds,
    ...
    for i, im_name in enumerate(im_names):
        ...
        plt.imsave(
            str(directory / f'{im_name}_view.png'),
            tools.show_segmentation(np.array(validation_ims[i]),
                                    np.array(preds[i]).astype(np.int16),
...
def cellpose_eval_from_ims(directory):
    ...
    thresholds = np.arange(0.5, 1.0, 0.05)
    APs, TPs, FPs, FNs = metrics.average_precision(mask,
                                                   pred,
                                                   threshold=thresholds)
    precisions = TPs / (TPs + FPs)
    recalls = TPs / (TPs + FNs)
    ...
    plt.imsave(
        str(directory / f'01_view.png'),
        tools.show_segmentation(np.array(image[0]),
                                np.array(pred[0]).astype(np.int16),
                                np.array(mask[0]).astype(np.int16)))


def plot_results(cellpose_results, rcnn_results):
    cellpose_results = pd.concat([
        pd.read_csv(file, sep='\t', index_col=0) for file in cellpose_results
    ],
                                 axis=0)
    rcnn_results = pd.concat(
        [pd.read_csv(file, sep='\t', index_col=0) for file in rcnn_results],
        axis=0)
    cellpose_means, cellpose_stds = cellpose_results.groupby(
        level=0).mean(), cellpose_results.groupby(level=0).std()
    rcnn_means, rcnn_stds = rcnn_results.groupby(
        level=0).mean(), rcnn_results.groupby(level=0).std()
    ...
    for ax, metric in zip(axs, metrics):
        ax.plot(thresholds,
                cellpose_means[metric],
                color='red',
                label='Cellpose')
        ax.fill_between(thresholds,
                        cellpose_means[metric] - cellpose_stds[metric],
                        cellpose_means[metric] + cellpose_stds[metric],
                        color='red',
                        alpha=0.5,
                        edgecolor='none')
        ax.plot(thresholds,
                ...
                label='Mask R-CNN')
        ax.fill_between(thresholds,
                        rcnn_means[metric] - rcnn_stds[metric],
                        rcnn_means[metric] + rcnn_stds[metric],
                        color='navy',
                        alpha=0.5,
                        edgecolor='none')
...
def plot_train_dat_results(directory):
    results = [
        ...
        for file in directory.iterdir() if file.suffix == '.txt'
    ]
    ...
    plt.legend()
    plt.savefig(
        '/home/ubuntu/Documents/detectron_and_track/cellpose_Models/test_training_data2/results.png'
    )
...
def main():
    ...
    plot_train_dat_results(
        Path(
            '/home/ubuntu/Documents/detectron_and_track/cellpose_Models/test_training_data2/validate'
        ))
