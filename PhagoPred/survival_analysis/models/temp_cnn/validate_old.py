...
def visualize_validation_predictions(
    ...
    for batch in dataloader:
        ...
        for i in range(len(batch['features'])):
            ...
            model.train()
            pred_single, attn_single = model(x.unsqueeze(0),
                                             length.unsqueeze(0),
            ...
            attn_vec = attn_single[
                0, -seq_len:].detach().cpu().numpy()  # (seq_len,)
            # attn_vec = attn_single[0].detach().cpu().numpy()
            feature_values = x.detach().cpu().numpy(
            ...
            cam_maps = [
                CAM(model, layer, batch['features'], batch['length'])
                for layer in model.cn_layers
            ]
            cam_map = torch.stack(cam_maps, dim=0).mean(dim=0)
            cam_vec = cam_map[i, -seq_len:].detach().cpu().numpy()

            saliency_map = smoothgrad_saliency_single(model,
                                                      x,
                                                      length,
                                                      n_samples=20,
                                                      sigma_ratio=0.1,
            ...
            ax_att.plot(np.arange(len(saliency_avg)),
                        saliency_avg,
                        color='tab:red',
                        label='SmoothGrad')
...
def average_attention(model, dataloader, device, save_as: Path, max_len=None):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc='Getting average attention scores'):
            ...
            _, attn_weights = model(cell_features,
                                    batch['length'],
                                    return_attention=True)
            ...
            for i in range(len(attn_weights_np)):
                if event_indicators[i] == 1:
                    ...
                    aligned_vec = np.concatenate(
                        [attn_vec,
                         np.full(int(time_to_event), np.nan)])

                    attn_accum.append(aligned_vec)
                    lengths_list.append(len(aligned_vec))
        max_len = max(lengths_list)
        padded_attn = [
            np.concatenate([np.full(max_len - len(attn), np.nan), attn])
            for attn in attn_accum
        ]
    ...
    plt.fill_between(np.arange(-len(avg_attn), 0),
                     lower,
                     upper,
                     color='k',
                     alpha=0.2,
...
def validate(model_, model_dir, val_hdf5_paths):
    ...
    average_attention(model_, validate_loader, device,
                      model_dir / 'attention_weights.jpeg')
    average_feature_importance(model_, validate_loader, device, features,
                               model_dir / 'feature_importance.jpeg')
