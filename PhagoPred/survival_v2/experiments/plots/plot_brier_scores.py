...
def plot_brier_scores(
    ...
    if len(varying_params) == 1:
        ...
        _plot_brier_on_ax(axs, all_experiments, varying_params,
                          percentile_range)
        return fig
    if len(varying_params) == 2:
        ...
        log.info(
            f'Skipping plotting Brier score curves, {len(varying_params)} varying paramaters'
        )
        return None


def _plot_brier_2var(
    all_experiments: list[ExperimentRecord],
    ...
    for i, outer_val in enumerate(outer_vals):
        ...
        _plot_brier_on_ax(axs[i], experiments, {inner_param_name: inner_vals},
                          percentile_range)

        axs[i].set_title(f'{outer_param_name}={outer_val.name}', fontsize=14)
        fig.suptitle('Brier Scores', fontsize=14, fontweight='bold')
...
def _plot_brier_on_ax(
    ...
    time_vals = np.linspace(0, max_time, 100)
    for i, val in enumerate(var_par_vals):
        ...
        plot_med_range_on_ax(ax, time_vals, interpolated_brier, cmap(i),
                             percentile_range, val.name)

    ax.set_title('Brier Scores', fontsize=14, fontweight='bold')
