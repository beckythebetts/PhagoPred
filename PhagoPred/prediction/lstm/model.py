...
class LSTM_Attention(torch.nn.Module):
    ...
    def __init__(self, input_size, lstm_hidden_size, lstm_dropout, attn_hidden_size, lstm_proj_size=0, bidirectional=True):
        """
        ...
        """
        super().__init__()
        self.hidden_size = lstm_hidden_size*2 if bidirectional else lstm_hidden_size
        ...
        self.attention1 = torch.nn.Linear(in_features=self.hidden_size, out_features=attn_hidden_size)
        self.attention2 = torch.nn.Linear(in_features=attn_hidden_size, out_features=1)
        self.classifier = torch.nn.Linear(in_features=self.hidden_size, out_features=1)
    ...
    def forward(self, x):
        ...
        """
        lstm_out, _ = self.lstm(x) # lstm_out [Sequence length, batch_size, hidden_size]
        attn_hidden = torch.nn.functional.relu(self.attention1(lstm_out)) # attn_hidden [sequence length, batch_size, hidden_size]
        attn_weights = torch.nn.functional.softmax(self.attention2(attn_hidden), dim=0) # attn_weights[sequence length, batch_size, 1]
        context_vector = torch.sum(attn_weights*lstm_out, dim=0) # context_vector [batch_size, hidden_size]
