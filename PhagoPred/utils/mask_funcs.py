...
def add_coco_annotation(coco_json: dict, image_name: str, mask: np.ndarray,
...
def coco_to_masks(coco_file: Union[Path, str],
                  im_name: str) -> dict[str, np.ndarray]:
    """
    ...
    masks = {
        category["id"]: np.zeros(shape=(height, width), dtype=np.int32)
            ...
            if isinstance(segmentation, list):
                ...
                for polygon in segmentation:
                    ...
                    outline_coords = outline_coords[:, [
                        1, 0
                    ]]  # swap x,y to y,x for mask indexing
                    poly_mask = skimage.draw.polygon2mask((height, width),
                                                          outline_coords)
                    instance_mask = np.logical_or(instance_mask, poly_mask)

                category_counts[category_id] += 1
                masks[category_id][instance_mask] = category_counts[
                    category_id]
...
def analyze_coco_annotations(coco_json_path):
    ...
    for cat_id, count in sorted(category_instance_counts.items(),
                                key=lambda x: x[1],
                                reverse=True):
...
def get_perimeters(mask, cell_batchsize=50):
    ...
    for first_cell in range(1, num_cells, cell_batchsize):
        ...
        kernel = torch.tensor([[1, 1, 1], [1, 9, 1], [1, 1, 1]]).to(device)
        # print(mask.shape, kernel.shape)
        padded_masks = torch.nn.functional.pad(expanded_mask, (1, 1, 1, 1),
                                               mode='constant',
                                               value=0)
        # print(padded_masks.shape)
        conv_result = torch.nn.functional.conv2d(
            padded_masks.squeeze().unsqueeze(1).float(),
            kernel.unsqueeze(0).unsqueeze(0).float(),
            padding=0).squeeze()
        perimeters[first_cell:last_cell] = torch.sum(
            (conv_result >= 10) & (conv_result <= 16), dim=(1, 2)).float()
    perimeters[0] = 0
...
def get_densities(mask, search_radius=500):
    ...
    distances = np.linalg.norm(np.expand_dims(centres, 1) -
                               np.expand_dims(centres, 0),
                               axis=(2))
...
def get_centres(mask, x_mesh_grid=None, y_mesh_grid=None):
    ...
    if x_mesh_grid is None or y_mesh_grid is None:
        x_mesh_grid, y_mesh_grid = np.meshgrid(np.arange(mask.shape[1]),
                                               np.arange(mask.shape[0]))
    # areas = get_areas(mask)
    centres = [
        np.array(get_centre(mask == idx, x_mesh_grid, y_mesh_grid))
...
def get_centre(mask, x_mesh_grid=None, y_mesh_grid=None):
    if x_mesh_grid is None or y_mesh_grid is None:
        x_mesh_grid, y_mesh_grid = np.meshgrid(np.arange(mask.shape[0]),
...
def to_masks(image_path, type):
    ...
    if type == 'proximity':
        ...
        return to_instance_mask(
            np.where(seg_mask[:, :, 2] == mask_vals[type], 1, 0))
...
def split_mask(mask_full, use_torch=False, return_indices=False):
    ...
    if use_torch:
        ...
        masks = [[np.where(mask_full == i + 1, 1, 0)]
                 for i in range(0, np.max(mask_full)) if i + 1 in mask_full]
    if return_indices:
        return masks_dict
    else:
        return masks
...
def create_circle(centre, radius, array_shape=(1024, 1024)):
    circle = circle_equ(np.arange(0, array_shape[0], 1),
                        np.arange(0, array_shape[1], 1)[:, np.newaxis], centre,
                        radius)
    return circle
...
def torch_circle(centre, radius, array_shape=SETTINGS.IMAGE_SIZE):
    circle = circle_equ(
        torch.arange(0, array_shape[0], 1).cuda().unsqueeze(1),
        torch.arange(0, array_shape[1], 1).cuda().unsqueeze(0), centre, radius)
    return circle
...
def mask_outline(mask, thickness=3):
    expanded_mask = (F.max_pool2d(mask.float().unsqueeze(0),
...
def mask_outlines(mask, thickness=3):
    expanded_mask = F.max_pool2d(mask.float().unsqueeze(0),
                                 kernel_size=2 * thickness + 1,
                                 stride=1,
                                 padding=thickness).to(torch.int16)
...
def get_crop_indices_all(centers, side_length, image_size):
    ...
    adjusted_y_end = np.where((y_end - y_start) < side_length,
                              y_start + side_length, y_end)
    adjusted_y_start = np.where((y_end - y_start) < side_length,
                                y_end - side_length, y_start)
    adjusted_x_end = np.where((x_end - x_start) < side_length,
                              x_start + side_length, x_end)
    adjusted_x_start = np.where((x_end - x_start) < side_length,
                                x_end - side_length, x_start)
...
def get_haralick_texture_features(image,
                                  mask,
    ...
    if mask is not None:
        ...
        try:
            ...
            image, mask = image[row_slice, col_slice], mask[row_slice,
                                                            col_slice]
            image = image * mask
        except ValueError:
            return features
    ...
    for i, distance in enumerate(distances):
        try:
            features[i] = mahotas.features.haralick(image,
                                                    distance=distance,
                                                    ignore_zeros=True,
                                                    return_mean=True)
...
def convert_coco_file(coco_file: Path,
                      im_dir: Path,
                      new_folder: Path,
                      categories: list[str] | str = 'all'):
    os.makedirs(new_folder, exist_ok=True)
    for im in im_dir.iterdir():
        masks = coco_to_masks(coco_file, im)
        if categories == 'all':
            categories = list(masks.keys())
        mask = combine_masks([v for k, v in masks.items()
                              if k in categories]).astype(np.uint16)
    ...
    for img in masks:
        ...
        for label in np.unique(img):
            ...
            for contour in contours:
                if len(
                        contour
                ) >= 5:  # Minimum number of points for an ellipse fitting
                    ...
                    diameters.append(
                        np.mean(ellipse[1])
                    )  # Take the average of the axes as the cell diameter

    # Return the average diameter of all cells
    return np.mean(diameters).astype(
        float)  # Fallback to a default if no cells are found
...
def clean_coco_json(coco_json_path: Path, image_dir: Path) -> None:
    ...
    existing_ims = [
        im for im in image_dir.iterdir() if im.suffix.lower() in image_exts
    ]
    ...
    valid_ims = [
        img for img in images if img['file_name'].lower() in existing_im_names
    ]
    valid_im_ids = set(img['id'] for img in valid_ims)
    valid_annotations = [
        ann for ann in annotations if ann['image_id'] in valid_im_ids
    ]
