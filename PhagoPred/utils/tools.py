...
def highlight_str(string: str) -> str:
    """Add === above and below string to highlight."""
    length = len(string)
    string = f'\n{"="*length}\n{string}\n{"="*length}'
    return string


def to_json_safe(obj: Dict) -> Dict:
...
def fill_missing_segs(seg_data: h5py.Dataset, first_appearance: np.ndarray,
    ...
    for i in tqdm(range(seg_data.shape[0]),
                  desc='Filling missing segmentations'):
        mask = seg_data[i, :, :]
...
def fill_missing_values(feature_ds: h5py.Dataset,
                        first_appearances: np.ndarray,
                        last_appearances: np.ndarray) -> None:
    """Interpolate the missing values for cell features"""
    for cell_idx in range(feature_ds.shape[1]):
        cell_array = feature_ds[:, cell_idx][:]
        first_appearance, last_appearance = first_appearances[
            cell_idx], last_appearances[cell_idx]
        valid_mask = ~np.isnan(cell_array)
        missing_idxs = np.nonzero(~valid_mask)[0]
        valid_idxs = np.nonzero(valid_mask)[0]
        missing_idxs = missing_idxs[(missing_idxs > first_appearance)
                                    & (missing_idxs < last_appearance)]

        try:
            cell_array[missing_idxs] = np.interp(missing_idxs, valid_idxs,
                                                 cell_array[valid_idxs])
        except ValueError:
            return
...
def fill_missing_cells(dataset: Path) -> None:
    with h5py.File(dataset, 'r+') as f:
        ...
        for feature_name in tqdm(list(f['Cells']['Phase'].keys()),
                                 desc="Interpolating feature values"):
            if f['Cells']['Phase'][feature_name].shape[0] > 1:
                if feature_name not in ['Macrophage', 'Dead Macrophage']:
                    fill_missing_values(f['Cells']['Phase'][feature_name],
                                        first_appearances, last_appearances)
...
def get_cell_end_frames(cell_idx: int,
                        h5py_file: h5py.File) -> tuple[int, int]:
...
def overlay_red_on_gray(gray: np.ndarray,
                        red_overlay: np.ndarray,
                        red_scale: float = 1,
    ...
    if normalize:
        gray = (gray - np.nanmin(gray)) / (np.nanmax(gray) - np.nanmin(gray) +
                                           1e-8)
        red_overlay = (red_overlay - np.nanmin(red_overlay)) / (
            np.nanmax(red_overlay) - np.nanmin(red_overlay) + 1e-8)
...
def hdf5_from_nd2(nd2_file: Path,
                  hdf5_file: Path,
                  phase_channel: int = 1,
                  epi_channel: int = 0,
                  start_frame: int = 0,
    ...
    with nd2.ND2File(nd2_file) as n:
        with h5py.File(hdf5_file, 'w') as h:
            ...
            Images.attrs[
                'Notes'] = 'Macrophages and staph bacteria, but no fluorescence'

            Images.attrs['Resolution / um'] = Images.attrs[
                'Pixel Size / um'] / Images.attrs['Objective magnification']
            Images.attrs['FOV / um'] = Images.attrs[
                'Image size / pixels'] * Images.attrs['Resolution / um']
...
def hdf5_from_tiffs(tiff_files_path: Path,
    ...
    with h5py.File(hdf5_file, 'w') as h:
        ...
        Images.attrs['Objective magnification'] = 20
        res_um = Images.attrs['Pixel Size / um'] / Images.attrs[
        ...
        Images.attrs[
            'Notes'] = "J774 macrphages, SH1000 S.Aureus MCherry, grown in TSB + 0.1% Tet overnight. Imaged in DMEM. 1*10**5 cells per dish 1:1 MOI."

        Phase = Images.create_group('Phase')
        Epi = Images.create_group('Epi')

        phase_ds = Phase.create_dataset("Data",
                                        shape=(0, Y, X),
                                        maxshape=(None, Y, X),
                                        dtype='uint8',
                                        chunks=(1, Y, X))
        epi_ds = Epi.create_dataset("Data",
                                    shape=(0, Y, X),
                                    maxshape=(None, Y, X),
                                    dtype='uint8',
                                    chunks=(1, Y, X))
        ...
        for i, file in enumerate(tiff_files):
            sys.stdout.write(
                f"\rProcessing file {i + 1} / {len(tiff_files)}; total frames: {total_frames}"
            )
            sys.stdout.flush()
            with tifffile.TiffFile(str(file)) as tif:
                pages = tif.pages
                ...
                for start_idx in range(0, num_pages // num_channels,
                                       batch_size):
                    end_idx = min(start_idx + batch_size,
                                  num_pages // num_channels)
                    current_batch_size = end_idx - start_idx

                    phase_batch = np.empty((current_batch_size, Y, X),
                                           dtype=np.uint16)
                    epi_batch = np.empty((current_batch_size, Y, X),
                                         dtype=np.uint16)
                    ...
                    # Write batches
                    phase_ds[total_frames:total_frames +
                             current_batch_size] = phase_uint8
                    epi_ds[total_frames:total_frames +
                           current_batch_size] = epi_uint8
...
def convert_to_uint8(image: np.ndarray,
                     min_val=None,
                     max_val=None) -> np.ndarray:
    """
...
def batch_convert_to_uint8(images: np.ndarray,
                           min_val=None,
                           max_val=None) -> np.ndarray:
    """
    ...
    if min_val is None:
        min_val = images.reshape(images.shape[0],
                                 -1).min(axis=1).reshape(-1, 1, 1)
    else:
        min_val = np.array(min_val, dtype=np.float32).reshape(-1, 1, 1)

    if max_val is None:
        max_val = images.reshape(images.shape[0],
                                 -1).max(axis=1).reshape(-1, 1, 1)
    else:
...
def show_segmentation(image_array,
                      mask_array,
                      true_mask_array=None,
                      thickness=3):
    if len(image_array.shape) == 2:
        image_array = np.stack((image_array, ) * 3, axis=-1)
    ...
    if true_mask_array is not None:
        ...
        true_outline = mask_funcs.mask_outlines(true_tensor,
                                                thickness=thickness)
        ...
        image_tensor[only_true] = torch.tensor([0, 255, 0],
                                               dtype=torch.uint8)  # Green
        image_tensor[only_pred] = torch.tensor([255, 0, 0],
                                               dtype=torch.uint8)  # Red
        image_tensor[overlap] = torch.tensor([255, 255, 0],
                                             dtype=torch.uint8)  # Yellow

    else:
        # Only predicted outline
        image_tensor[pred_outline > 0] = torch.tensor([255, 0, 0],
                                                      dtype=torch.uint8)
...
def show_semantic_segmentation(image_array, mask_array, true_mask_array=None):
    if len(image_array.shape) == 2:
        image_array = torch.stack((image_array, image_array, image_array),
                                  dim=-1)
    ...
    with h5py.File(src_filename,
        ...
        for group_name in groups_to_copy:
            if group_name in src_file:
                ...
                print(
                    f"Warning: Group '{group_name}' not found in the source file."
                )
...
def crop_hdf5(file, crop_size=[2048, 2048]):
    ...
    with h5py.File(file, 'r+') as f:
        x_centre, y_centre = (int(
            f['Images'].attrs['Image size / pixels'][0] /
            2), int(f['Images'].attrs['Image size / pixels'][1] / 2))

        f['Images'].attrs['Image size / pixels'] = crop_size

        for modality in ('Phase', 'Epi'):
            for frame in list(f['Images'][modality].keys()):
                cropped_im = f['Images'][modality][frame][
                    int(x_centre - crop_size[0] / 2):int(x_centre +
                                                         crop_size[0] / 2),
                    int(y_centre - crop_size[1] / 2):int(y_centre +
                                                         crop_size[1] / 2)]
...
def create_hdf5(hdf5_filename, phase_tiffs_path, epi_tiffs_path):
    ...
    with h5py.File(hdf5_filename, 'w') as f:
        ...
        for im in Path(phase_tiffs_path).iterdir():
            ...
            sys.stdout.flush()
            Phase.create_dataset(im.stem,
                                 data=np.array(Image.open(im, mode='r')))
            i += 1
...
def rename_datasets_in_group(h5_file_path, group_path):
    ...
    with h5py.File(h5_file_path, 'r+') as h5_file:
        ...
        for old_name in original_dataset_names:
            ...
            if new_name == "":
                raise ValueError(
                    f"The dataset name '{old_name}' would become empty after renaming."
                )
...
def get_images(h5file, mode, first_frame=0, last_frame=SETTINGS.NUM_FRAMES):
    return np.array([
        h5file['Images'][mode][frame][:] for frame in list(
            h5file['Images'][mode].keys())[first_frame:last_frame]
    ],
                    dtype='uint8')
...
def get_masks(h5file, mode, first_frame=0, last_frame=SETTINGS.NUM_FRAMES):
    return np.array([
        h5file['Segmentations'][mode][frame][:] for frame in list(
            h5file['Segmentations'][mode].keys())[first_frame:last_frame]
    ],
                    dtype='int16')
...
def threshold_image(frames: np.ndarray,
                    lower_percentile: float = 99,
                    upper_percentile: float = 99.9) -> np.ndarray:
    ...
    return np.where((frames >= lower_threshold) & (frames <= upper_threshold),
                    frames, 0)
...
def plot_tracks(save_as):
    ...
    with h5py.File(SETTINGS.DATASET, 'r') as f:
        ...
        for cell_idx in range(1, xs.shape[1]):
            colour = torch.tensor(np.random.uniform(0, 2**(8) - 1, size=3),
                                  dtype=torch.uint8).to(device)
            ...
            for i in range(len(xsi) - 1):
                tracks_plot = draw_line(tracks_plot, xsi[i], xsi[i + 1],
                                        ysi[i], ysi[i + 1], colour)
...
def crop_bbox_to_fit(bbox, crop_x, crop_y, crop_width, crop_height):
    ...
    cropped_w = min(bbox[0] + bbox[2], crop_x + crop_width) - max(
        bbox[0], crop_x)
    cropped_h = min(bbox[1] + bbox[3], crop_y + crop_height) - max(
        bbox[1], crop_y)
...
def split_coco_by_category(coco_file, save_dir, category=None):
    ...
    with open(coco_file, 'r') as f:
        coco = json.load(f)
    categories = coco['categories'] if category is None else [
        cat for cat in coco['categories'] if cat['name'] == category
    ]
...
def split_image_and_coco(image_dir,
                         coco_path,
                         new_image_dir,
                         new_coco_path,
                         x_splits=2,
    ...
    for image in coco['images']:
        ...
        for i, x in enumerate(x_split_coords):
            for j, y in enumerate(y_split_coords):
                ...
                plt.imsave(new_image_dir / cropped_image_name,
                           cropped_image,
                           cmap='gray')
                ...
                for annotation in coco['annotations']:
                    if annotation['image_id'] == image['id']:
                        ...
                        cropped_bbox = crop_bbox_to_fit(
                            bbox, x, y, x_size, y_size)
                        ...
                        if cropped_bbox:
                            ...
                            if 'segmentation' in new_annotation:
                                ...
                                for poly in new_annotation['segmentation']:
                                    cropped_poly = crop_polygon_to_fit(
                                        poly, x, y, x_size, y_size)
                            ...
                            if new_annotation.get(
                                    'bbox') and new_annotation.get(
                                        'segmentation'):
...
def coco_to_semantic_training_data(image_dir, coco_json, target_im_dir,
                                   target_mask_dir):
    ...
    with h5py.File(SETTINGS.DATASET, 'r') as f:
        ...
        for i, f_i in enumerate(feature_idxs):
            for j, f_j in enumerate(feature_idxs):
                ...
                features_i, features_j = remove_outliers(
                    features_df[:, :, f_i].flatten()), remove_outliers(
                        features_df[:, :, f_j].flatten())
...
def sample_phase_images(datasets_dir: Path,
                        output_dir: Path,
        ...
        try:
            with h5py.File(h5_path, 'r') as f:
                ...
                if isinstance(phase_ds, h5py.Dataset):
                    ...
                    indices = random.sample(range(num_frames),
                                            min(n_samples, num_frames))
                    for idx in indices:
                        img = phase_ds[idx]
                        _save_phase_jpeg(img, output_dir, h5_path, idx)
                elif isinstance(phase_ds, h5py.Group):
                    frame_keys = sorted(phase_ds.keys())
                    chosen = random.sample(frame_keys,
                                           min(n_samples, len(frame_keys)))
                    for key in chosen:
                        img = phase_ds[key][:]
...
def _save_phase_jpeg(img: np.ndarray, output_dir: Path, h5_path: Path,
                     frame_id) -> None:
...
if __name__ == '__main__':
    ...
    sample_tiff_images(
        '/home/ubuntu/thor_server',
        '/home/ubuntu/PhagoPred/PhagoPred/Datasets/sample_tiff_images')
